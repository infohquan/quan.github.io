...
function TaoListMuti(lll,divid, hdf, key_)
{
	...
	if(lll!=""&&lll!=null)
	{
		var tpl__="";
		if(key_=="suburb")
		    tpl__ = $.template("<li><a href=\"javascript:;\"  onclick=\"SetTextSuburb();LoadEstate($('#txtStateID').val());WriteTextCondition();\"><input type=\"checkBox\" value=\"${eValue}\" title=\"${eName}\" onclick=\"SetTextSuburb();LoadEstate($('#txtStateID').val());WriteTextCondition();\"/> ${eName}</a></li>");
		if(key_=="kieubds")
...
function getDataEstate()
{
	...
	strEstate="";
	// tao text quan huyen
	$('#divEstate input:checkbox').each(function (){
		if(this.checked==true)	
		{			
...
function LoadEstate(stateid)
{
    if (stateid == "") return;
	if (stateid != $('#txtStateID').val() || listEstateFolState.length == 0)
	{
		var url = pathClientAjax+"handler/FormSearchHandler.aspx@act=LoadEstate&suburb="+$("#txtSuburb").val()+"&stateid="+stateid+"&ttype="+$('#txtTranClass').val()+"&ptype="+$('#txtProClass').val()+"&d="+(new Date()).getTime();
		$.post(url, {},
			function (data) {
...
{	
	...
	if(lll!=""&&lll!=null)
	{
	  //  $('#' + divid).append("<li style=\"width:570px; border-bottom:1px solid #FFF2E4;margin-bottom:5px;\"><a href=\"javascript:;\" onclick=\"$('#divEstate input:checkbox').removeAttr('checked'); getDataEstate(); WriteTextCondition();\">Xóa tất cả</li>");
	    var tpl__ = $.template("<li><a href=\"javascript:;\" onclick=\"getDataEstate();WriteTextCondition();\"><input type=\"checkbox\" value=\"${eID}\" title=\"${eName}\" onclick=\"getDataEstate();WriteTextCondition();\" >${eName} (${eCount})</a></li>");		
		for(var i =0;i<lll.length;i++)
		{
			if(lll[i].s4!="0")
			{
...
function SubmitFormSearchAdvance(proclass,transclass,stateid)
{
...
var NvgFst = {
    DefaultGiaTTDt: function () {
       ...
       // ShowTextTopSelect("hrfLoaiBDS", "iptLoaiBDS", $('#txtProClassText').val())
        ...
        var url = pathClientAjax + "handler/formsearchhandler.aspx@act=gttdt_default&d=" + (new Date()).getTime();
        $.post(url, { tc: tc, pc: pc },
		function (data) {
