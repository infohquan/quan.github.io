...
function changeLoai(loai_,obj_id,value_){	
	...
	var url = pathClientAjax+"handler/Alert.aspx@act=load_loai_bds&id="+loai_;	
...
{
	var url = pathClientAjax+"handler/FormSearchHandler.aspx@act=LoadPriceAlert&t="+tt;
	$.get(url, {},
		function(data){
			var lcNodes     = data.getElementsByTagName("*")[0].childNodes;		
			if(lcNodes.length > 0){						
...
function LoadEstate(stateid,divid)
{
	if(stateid!="")
	{
		...
		var url = pathClientAjax+"users/UsrHandler.aspx@act=loadestatemyalert&suburb="+$('#txtSuburb').val()+"&stateid="+stateid+"&ttype="+$('#txtSuburb').val()+"&ptype="+$('#txtSuburb').val()+"&d="+(new Date()).getTime();
